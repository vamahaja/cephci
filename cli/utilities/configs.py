...
def get_subscription_credentials():
    """Get subscription manager details from config"""
    _dict = {}
    log.info(f"Loading subscription manager details for subscription manager")
    try:
        _subscription = _get_credentials()["subscription"]
        _serverurl = _dict["serverurl"] = _subscription["serverurl"]
        _dict["baseurl"] = _subscription["baseurl"]
        _dict["username"] = _subscription["username"]
        _dict["password"] = _subscription["password"]
        _dict["timeout"] = int(_subscription.get("timeout", 60))
        _dict["retry"] = int(_subscription.get("retry", 6))

        log.info(f"Loaded subscription manager details for - {_serverurl}")
    except KeyError:
        raise ConfigError(f"Insufficient config for subscription manager")

    return _dict


def get_registry_credentials(build):
    ...
    """
    _dict = {}
    log.info(f"Loading registry credentials for server for build '{build}'")
    try:
        # Get registry credentials
        _registry = _get_credentials()["registry"][build]
        _serverurl = _dict["registry"] = _registry["registry"]
        _dict["username"] = _registry["username"]
        _dict["password"] = _registry["password"]

        log.info(f"Loaded registry credentials for server - {_serverurl}")
    except KeyError:
        raise ConfigError(f"Insufficient config for '{build}' registry")
...
def get_repos(build, version):
    """Get repositories from config for server and version

    Args:
        build (str): Build type production or released
        version (str): Distro version
    """
    log.info(f"Loading repositories for '{build}' and distro version '{version}'")
    try:
        _repo = _get_repos()[build]
        return _repo[version]
    except KeyError:
        raise ConfigError(f"Insufficient config for {build} & '{version}' in repo")
